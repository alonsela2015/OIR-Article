; Created by Alon Sela to inspect the spread by spreading groups. 2018
; The model is based (modified from) on existing models, created by by Uri Wilenski, then modified by Lada Adamic, and Eytan Bakshy, see copywrite below.
...
; Here we make the spreading group
...
; here we continue constructing the network
...
; for terms of use.
; This model was adopted from:
; models library by Lada Adamic (see copyright below)
...
@#$#@#$#@
## SPREADING GROUP MODEL

"Spreading Group" - (Based on a Lada and Eytan's modification of the default BA model that comes with the NetLogo, see detailed copyrights below).

The initial choice of nodes to seed, can be a set of nodes from 
(1) k nodes which are randomly selected.  
(2) k nodes from the spreading group sub-graph
(3) k nodes with the highest eigenvalue centrality 

## WHAT IS IT?

The model demonstrate how the use of spreading groups help the spread of a message.


## HOW IT WORKS
Creation of network:
The model starts with two nodes connected by an edge.
At each step, a new node is added.  after n/2 nodes were added, a step of formation of the spreading gorup is performed. This is an addition of links between nodes in the spreading group.
Following this process, another n/2 steps of nodes creation and connections is performed by a preferential attachment BA process.

The retention loss is modified by the slider "size-reduction", the density of the spreafing group is modified by the slider "spreadingGroupRepet" and the percentage of the spreading group from the entire network is modified by the slider "spreading-group-percent".
## HOW TO USE IT
...
The m slider defines the number of edges added at each step.

Choose num-nodes and gamma and press SETUP.

Adjust the p value to determine the infectiousness of the spreading agent. 
Press REINFECT ONCE to re-infect an initial set of nodes. This set can be from one of the methods in the REINFECT-TYPE choser.

RANDOM - k randomely selected nodes
Spreading  - k nodes from the spreading group
Eigen - k nodes withthe highest eigenvalue centrality.


Chose the retention loss on/off as well as its speed (i.e. size reduction.)\
The size of the node is proportinal to its likelyhood to infect.
